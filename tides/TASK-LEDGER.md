...
| E9 | 2026-07-19 | HSE06 hybrid SCF test + PP source-tree paths | TestHSE06Hybrid failed because E_hse_correction was only computed when SCF converged; removed that guard. The crude two-center V_x_SR folded into the SCF Hamiltonian made HSE06 SCF unstable (energy −2132 Ha on some runs), so it is now reported as a post-SCF correction only. Also made nao_driver_tests, real_pseudodojo_tests, and cuda_pp_build_tests locate bundled PPs from the source tree so ctest works from the build directory. | PASS for HSE gate | nao_driver_tests executable ALL PASS (E_hse_correction = −0.0633 Ha, energy = −2.842 Ha). Known remaining failures: real_pseudodojo_scf (Si atom PP gives positive +2.2 Ha, SCF not converged) and nao_pseudo_scf_validation (H2 trivial PP energy differs from AE by 6.3 Ha); both appear to be pre-existing pseudo/basis issues outside this scope. |
