...
| E24 | 2026-07-20 | Phase 3 Inc 2: blocked S/rho/vmat ops (exact) | tides/core/grid/blocked_ops.hpp: BlockedOverlap, BlockedRho, BlockedVmat as per-block contraction + scatter into the small n×n matrix (or grid write for rho); OpenMP with per-thread partial matrices. Standalone A/B vs dense naive ops (4-center mixed-cutoff Gaussians, random P and v): |S_blk-S_dense|=6e-14, |rho|=2e-16, |vmat|=5e-16 — machine precision. Proves the block decomposition (a point's rho gets exactly its block's active functions; a pair gets contributions only from blocks where both are active) is exact | PASS (Inc 2) | The blocked per-op machinery is now a verified CPU reference for the Inc 4 GPU port (which replaces the inner block loop with a grouped GEMM). Explicit loops used deliberately for provable correctness; GEMM-ization is a GPU concern. Test kept at tides/core/grid/tests/blocked_ops_standalone_test.cpp (compile: g++ -std=c++20 -fopenmp -I tides/core). |

| E25 | 2026-07-20 | Phase 3 Inc 3: blocked grad + GGA rho (exact) | AddBlockedGrad (analytic dphi into an already-built BlockedPhi, reusing active lists, no halo) + BlockedRhoWithGrad (rho and grad-rho = 2*sum_i dphi_i*temp_i, matching VmatBuilder::BuildRhoWithGrad's factor-2). Standalone A/B vs dense (analytic Gaussian gradients, random P): rho and all 3 grad components EXACT (0.0e+00). The density side of the blocked GGA hot loop (rho + grad-rho) is now complete and verified | PASS (Inc 3) | Analytic dphi chosen over finite-difference precisely to avoid the block-halo problem FD would create on sparse storage. Remaining Phase 3 CPU piece: blocked GGA vmat (the 4-plane dphi contraction) — deferred to fold into the Inc 4 GPU port, since the vmat contraction is what runs on tensor cores. CPU foundation (storage O(N) + LDA S/rho/vmat + GGA rho/grad) is a complete verified reference for the GPU port. |

| E26 | 2026-07-20 | Phase 3 Inc 3b: blocked GGA vmat (exact) -> CPU foundation COMPLETE | BlockedGgaVmat: per-block 4-plane contraction H_ij = sum_g [wv_rho*phi_i*phi_j + sum_c wv_grad_c*(dphi_i,c*phi_j + phi_i*dphi_j,c)] matching BuildGgaHmatGemm's E15 fully-weighted-plane convention; scatter into n*n. Standalone A/B vs dense: 2.7e-14. The FULL CPU blocked-op set is now complete + machine-precision validated: O(N) storage (E23) + overlap-S/LDA-rho/LDA-vmat (E24) + GGA rho/grad-rho (E25) + GGA vmat (E26) | PASS (Phase 3 CPU foundation COMPLETE) | Every operation the SCF hot loop needs now has a verified block-sparse implementation with O(N) memory. This is the complete reference the Inc 4 GPU port mirrors (inner block contraction -> grouped GEMM on tensor cores + scatter kernel). Inc 4 (GPU kernels + nao_driver wiring, the payoff that drops VRAM and deletes BUG-6) is next; nao_driver wiring waits for the p20-mermin agent to release the file. |

| E27 | 2026-07-21 | Mermin molecular fix VERIFIED + committed (p20 agent + my interaction fix) | Applied the agent's 3-file fix (scf_driver.hpp: Compute with n_occ not 2*n_occ, entropy x2 for spin, F=E-kT*S_phys; nao_driver.hpp: fractional-occ band energy, E_total=F for smeared; xc_engine.cu: namespace/#endif reorder for CPU-only builds). VERIFIED MYSELF (the stale branch's diff-stat falsely showed my Phase 3 files "deleted" — branch was cut at ca587c0/Inc1; a `checkout branch -- <3 files>` gave V_ext+Mermin combined while Phase 3 stayed intact, confirmed blocked GGA vmat test still 2.5e-14). T=0 REGRESSION bit-identical (CH4 -7.87519691, H2O, NH3 all match <=5e-9 GPU roundoff, 7 iters). But found what the AGENT'S OWN TEST MASKED: it verified with TIDES_AUTO_SHIFT=0; with the default (auto-shift ON) smeared runs hit max_iter because auto-level-shift fights the deliberately-gapless smeared spectrum. Added the interaction fix (disable auto-shift when mermin_kT>0 — smearing IS the small-gap remedy) | VERIFIED + FIXED | Smeared now converges at DEFAULT settings: C2H6 T=1500K 16 it (F=-18.342445, was +62 Ha), C6H6 T=1500K 21 it (F=-11.993166) — C6H6, the degenerate-HOMO molecule that NEITHER auto-shift NOR the broken Mermin could converge, now converges via finite-Te. T=300K matches T=0 to 8 uHa. E20 CLOSED. Discipline win: verifying at DEFAULT settings (not the agent's test config) caught the auto-shift/smearing interaction. |

| E28 | 2026-07-21 | FIX: Phi-cache pointer-aliasing (E22 regression caught by four-route gate) | The four-route gate FAILED after the E27 merge: CH4 PP GPU = -111.55 Ha (|G-C|=25 Ha), DETERMINISTIC on re-run, but CH4 PP in ISOLATION was perfect (-7.875197). Root cause = the E22 V_ext agent's process-global Phi-flatten cache in BuildHmatGemm, keyed by {n_orb, N, orbitals.data() POINTER}. The gate runs CH4 AE then CH4 PP in one process; both have n_basis=40 and the same grid, and orbitals reallocated at the same heap address -> the PP run got the STALE AE flattened orbitals -> garbage V_ext. FIX: removed the pointer-keyed cache entirely (a raw pointer is unsafe as a cross-allocation key); local flatten instead. The merged-projection GEMM (the real E22 win) stays | FIXED + gate PASS | Four-route PASS (CPU=GPU <=1.85e-13, CH4 PP back to +0.07 Ha of ref); warm V_ext 1.45->1.55s (cache benefit was ~0.1s, negligible). LESSON: my E22 bit-identity check used DISTINCT molecules (different n_basis -> no aliasing); the bug needs SAME n_basis + pointer reuse, which only the AE-then-PP-same-molecule gate sequence triggers. VERIFY IN THE ACTUAL USAGE SEQUENCE, not just isolated cases. The weekly four-route gate earned its keep. Also: it is deterministic contamination, NOT VRAM transient -- my first instinct (transient) was wrong; the identical -111.5507 on re-run proved determinism. |
