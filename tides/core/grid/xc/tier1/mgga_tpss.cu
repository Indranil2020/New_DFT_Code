...
namespace mgga_x_tpss {
struct mgga_x_tpss_params {
...
#include "/home/niel/src/libxc-7.0.0/src/maple2c/mgga_exc/mgga_x_tpss.c"
}  // namespace mgga_x_tpss

namespace mgga_c_tpss {
struct mgga_c_tpss_params {
  double beta, d;
  double C0_c[4];
};
#define XC_DONT_COMPILE_EXC
...
#define XC_DONT_COMPILE_KXC
#define XC_DONT_COMPILE_LXC
#include "/home/niel/src/libxc-7.0.0/src/maple2c/mgga_exc/mgga_c_tpss.c"
}  // namespace mgga_c_tpss

namespace {

__device__ MggaEvaluation MggaTpssEval(double rho, double sigma, double tau) {
  using tides::grid::xc::MggaEvaluation;

  // Exchange parameters (TPSS defaults)
  mgga_x_tpss::mgga_x_tpss_params x_params = {
      0.40, 1.59096, 1.537, 0.8040, 0.21951, 2.0, 0.0};
  // Correlation parameters (TPSS defaults)
  mgga_c_tpss::mgga_c_tpss_params c_params = {
  ...
  xc_func_type x_func = {};
  x_func.info = &x_info;
  x_func.nspin = 1;  // unpolarized
  x_func.dens_threshold = 1e-15;
  x_func.zeta_threshold = DBL_EPSILON;
  x_func.sigma_threshold = 1e-20;  // pow(dens_threshold, 4.0/3.0)
  ...
  c_func.sigma_threshold = 1e-20;  // pow(dens_threshold, 4.0/3.0)
  ...
  mgga_x_tpss::func_vxc_unpol(&x_func, 0, &eval_rho, &eval_sigma, &lapl, &eval_tau, &x_out);
  mgga_c_tpss::func_vxc_unpol(&c_func, 0, &eval_rho, &eval_sigma, &lapl, &eval_tau, &c_out);
...
struct MggaTpssPolFunctor {
  __device__ MggaPolEvaluation operator()(const double* rho, const double* sigma,
    ...
    mgga_x_tpss::func_vxc_pol(&x_func, 0, my_rho, my_sigma, lapl, my_tau, &x_out);
    ...
    mgga_c_tpss::func_vxc_pol(&c_func, 0, my_rho, my_sigma, lapl, my_tau, &c_out);
...
void MggaTpssKernel(const double* __restrict__ rho,
  ...
  for (std::int64_t point = static_cast<std::int64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
    ...
    MggaEvaluation evaluation = MggaTpssEval(density, sigma, tau[point]);
