...
#include "/home/niel/src/libxc-7.0.0/src/maple2c/mgga_exc/mgga_x_scan.c"
}  // namespace mgga_x_scan
...
#define XC_DONT_COMPILE_KXC
#define XC_DONT_COMPILE_LXC
#include "/home/niel/src/libxc-7.0.0/src/maple2c/mgga_exc/mgga_c_scan.c"
}  // namespace mgga_c_scan

namespace {

__device__ MggaEvaluation MggaScanEval(double rho, double sigma, double tau) {
...
void MggaScanKernel(const double* __restrict__ rho,
  ...
  for (std::int64_t point = static_cast<std::int64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
    ...
    MggaEvaluation evaluation = MggaScanEval(density, sigma, tau[point]);
...
__global__ void MggaScanDeterministicEnergyKernel(const double* __restrict__ rho,
                                                  const double* __restrict__ grad,
                                                  const double* __restrict__ tau,
                                                  const double* __restrict__ weights,
                                                  double* __restrict__ exc,
...
Status LaunchMggaScan(const XcGridIn& input, XcGridOut& output,
  ...
  if (nspin == 2) {
    MggaPolKernelScalar<MggaScanPolFunctor><<<blocks, kThreads, 0, stream>>>(
    ...
    if (cudaGetLastError() != cudaSuccess) return Status::IoError("MggaScanPolKernelScalar launch");
    MggaPolKernelGrad<MggaScanPolFunctor><<<blocks, kThreads, 0, stream>>>(
        input.rho, input.grad, input.tau, input.w, output.wv_grad,
        input.np, input.point_stride);
    if (cudaGetLastError() != cudaSuccess) return Status::IoError("MggaScanPolKernelGrad launch");
    if (!deterministic) return Status::Ok();
    MggaPolDeterministicEnergyKernel<MggaScanPolFunctor><<<1, 1, 0, stream>>>(
        input.rho, input.grad, input.tau, input.w, output.exc_per_system,
        input.np, input.point_stride);
    if (cudaGetLastError() != cudaSuccess) return Status::IoError("MggaScanPolDeterministicEnergyKernel launch");
    return Status::Ok();
  }
  MggaScanKernel<<<blocks, kThreads, 0, stream>>>(
      input.rho, input.grad, input.tau, input.w, output.wv_rho,
      output.wv_grad, output.wv_tau, output.exc_per_system, input.np,
      input.point_stride, !deterministic);
  if (cudaGetLastError() != cudaSuccess) return Status::IoError("MggaScanKernel launch");
  if (!deterministic) return Status::Ok();
  MggaScanDeterministicEnergyKernel<<<1, 1, 0, stream>>>(
      input.rho, input.grad, input.tau, input.w, output.exc_per_system,
      input.np, input.point_stride);
  if (cudaGetLastError() != cudaSuccess) return Status::IoError("MggaScanDeterministicEnergyKernel launch");
  return Status::Ok();
}
