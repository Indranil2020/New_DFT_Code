#include "grid/xc/functionals/mgga_scan.cuh"
...
using tides::grid::xc::MggaScanFunctor;
using tides::grid::xc::MggaScanPolFunctor;
...
void MggaScanKernel(const double* __restrict__ rho,
  ...
  for (std::int64_t point = static_cast<std::int64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
    ...
    MggaEvaluation evaluation = MggaScanFunctor::Eval(density, sigma, tau[point]);
