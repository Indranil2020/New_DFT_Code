#include "grid/xc/functionals/mgga_r2scan.cuh"
...
using tides::grid::xc::MggaR2scanFunctor;
using tides::grid::xc::MggaR2scanPolFunctor;
...
void MggaR2scanKernel(const double* __restrict__ rho,
  ...
  for (std::int64_t point = static_cast<std::int64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
    ...
    MggaEvaluation evaluation = MggaR2scanFunctor::Eval(density, sigma, tau[point]);
