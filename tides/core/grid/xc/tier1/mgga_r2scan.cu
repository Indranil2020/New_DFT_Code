...
__device__ MggaEvaluation MggaR2scanEval(double rho, double sigma, double tau) {
  using tides::grid::xc::MggaEvaluation;

  mgga_x_r2scan::mgga_x_r2scan_params x_params = {
      0.667, 0.8, 1.24, 0.065, 0.001, 0.361};
  mgga_c_r2scan::mgga_c_r2scan_params c_params = {0.001};

  xc_func_info_type x_info = {};
  x_info.flags = XC_FLAGS_3D | XC_FLAGS_NEEDS_TAU | XC_FLAGS_HAVE_EXC | XC_FLAGS_HAVE_VXC;
  x_info.dens_threshold = 1e-11;
  ...
  mgga_x_r2scan::func_vxc_unpol(&x_func, 0, &eval_rho, &eval_sigma, &lapl, &eval_tau, &x_out);
  mgga_c_r2scan::func_vxc_unpol(&c_func, 0, &eval_rho, &eval_sigma, &lapl, &eval_tau, &c_out);
...
struct MggaR2scanPolFunctor {
  __device__ MggaPolEvaluation operator()(const double* rho, const double* sigma,
    ...
    mgga_x_r2scan::func_vxc_pol(&x_func, 0, my_rho, my_sigma, lapl, my_tau, &x_out);
    ...
    mgga_c_r2scan::func_vxc_pol(&c_func, 0, my_rho, my_sigma, lapl, my_tau, &c_out);
...
void MggaR2scanKernel(const double* __restrict__ rho,
  ...
  for (std::int64_t point = static_cast<std::int64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
    ...
    MggaEvaluation evaluation = MggaR2scanEval(density, sigma, tau[point]);
...
__global__ void MggaR2scanDeterministicEnergyKernel(const double* __restrict__ rho,
                                                    const double* __restrict__ grad,
                                                    const double* __restrict__ tau,
                                                    const double* __restrict__ weights,
                                                    double* __restrict__ exc,
...
Status LaunchMggaR2scan(const XcGridIn& input, XcGridOut& output,
  ...
  if (nspin == 2) {
    MggaPolKernelScalar<MggaR2scanPolFunctor><<<blocks, kThreads, 0, stream>>>(
    ...
    if (cudaGetLastError() != cudaSuccess) return Status::IoError("MggaR2scanPolKernelScalar launch");
    MggaPolKernelGrad<MggaR2scanPolFunctor><<<blocks, kThreads, 0, stream>>>(
        input.rho, input.grad, input.tau, input.w, output.wv_grad,
        input.np, input.point_stride);
    if (cudaGetLastError() != cudaSuccess) return Status::IoError("MggaR2scanPolKernelGrad launch");
    if (!deterministic) return Status::Ok();
    MggaPolDeterministicEnergyKernel<MggaR2scanPolFunctor><<<1, 1, 0, stream>>>(
        input.rho, input.grad, input.tau, input.w, output.exc_per_system,
        input.np, input.point_stride);
    if (cudaGetLastError() != cudaSuccess) return Status::IoError("MggaR2scanPolDeterministicEnergyKernel launch");
    return Status::Ok();
  }
  MggaR2scanKernel<<<blocks, kThreads, 0, stream>>>(
      input.rho, input.grad, input.tau, input.w, output.wv_rho,
      output.wv_grad, output.wv_tau, output.exc_per_system, input.np,
      input.point_stride, !deterministic);
  if (cudaGetLastError() != cudaSuccess) return Status::IoError("MggaR2scanKernel launch");
  if (!deterministic) return Status::Ok();
  MggaR2scanDeterministicEnergyKernel<<<1, 1, 0, stream>>>(
      input.rho, input.grad, input.tau, input.w, output.exc_per_system,
      input.np, input.point_stride);
  if (cudaGetLastError() != cudaSuccess) return Status::IoError("MggaR2scanDeterministicEnergyKernel launch");
  return Status::Ok();
}
