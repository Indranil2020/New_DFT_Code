...
int Fail(const char* message) {
  std::cerr << "xc_mgga_scan_device: " << message << '\n';
  return 1;
}
...
int main() {
  ...
  if (!exchange.Init(kLibxc_MGGA_X_SCAN, XC_UNPOLARIZED) ||
      !correlation.Init(kLibxc_MGGA_C_SCAN, XC_UNPOLARIZED)) {
    return Fail("failed to initialize the libxc SCAN oracle");
  }
  ...
  if (cudaMemcpyAsync(arena.rho(), rho.data(), scalar_bytes, cudaMemcpyHostToDevice,
      ...
      cudaMemcpyAsync(arena.tau(), tau.data(), scalar_bytes, cudaMemcpyHostToDevice,
                      stream) != cudaSuccess) {
    return Fail("SCAN input upload failed");
  }

  XcSpec spec;
  spec.family = Family::kMgga;
  spec.nspin = 1;
  spec.terms = {{Functional::kScan, 1.0}};
  ...
  if (cudaMemcpyAsync(observed_wv_rho.data(), arena.wv_rho(), scalar_bytes,
    ...
    return Fail("SCAN output download failed");
  }
  ...
  for (int run = 0; run < 100; ++run) {
    ...
    if (cudaMemcpyAsync(&deterministic_energy, arena.exc_per_system(), sizeof(double),
                        cudaMemcpyDeviceToHost, stream) != cudaSuccess ||
        cudaStreamSynchronize(stream) != cudaSuccess) {
      return Fail("deterministic SCAN energy download failed");
    }
    ...
    if (run == 0) {
      ...
      return Fail("deterministic SCAN energy changed across repeated runs");
    }
  }
  ...
  std::cout << "xc_mgga_scan_device: points=" << rho.size()
            << " stride=" << stride
  ...
  if (!all_wv_rho_match || !all_wv_grad_match || !all_wv_tau_match ||
    ...
    return Fail("device SCAN weighted outputs differ from the libxc oracle");
  }
  return 0;
}
