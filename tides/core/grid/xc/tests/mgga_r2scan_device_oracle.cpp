...
int Fail(const char* message) {
  std::cerr << "xc_mgga_r2scan_device: " << message << '\n';
  return 1;
}
...
int main() {
  ...
  if (!exchange.Init(kLibxc_MGGA_X_R2SCAN, XC_UNPOLARIZED) ||
      !correlation.Init(kLibxc_MGGA_C_R2SCAN, XC_UNPOLARIZED)) {
    return Fail("failed to initialize the libxc r2SCAN oracle");
  }
  ...
  if (cudaMemcpyAsync(arena.rho(), rho.data(), scalar_bytes, cudaMemcpyHostToDevice,
      ...
      cudaMemcpyAsync(arena.tau(), tau.data(), scalar_bytes, cudaMemcpyHostToDevice,
                      stream) != cudaSuccess) {
    return Fail("r2SCAN input upload failed");
  }

  XcSpec spec;
  spec.family = Family::kMgga;
  spec.nspin = 1;
  spec.terms = {{Functional::kR2scan, 1.0}};
  ...
  if (cudaMemcpyAsync(observed_wv_rho.data(), arena.wv_rho(), scalar_bytes,
    ...
    return Fail("r2SCAN output download failed");
  }
  ...
  for (int run = 0; run < 100; ++run) {
    ...
    if (cudaMemcpyAsync(&deterministic_energy, arena.exc_per_system(), sizeof(double),
                        cudaMemcpyDeviceToHost, stream) != cudaSuccess ||
        cudaStreamSynchronize(stream) != cudaSuccess) {
      return Fail("deterministic r2SCAN energy download failed");
    }
    ...
    if (run == 0) {
      ...
      return Fail("deterministic r2SCAN energy changed across repeated runs");
    }
  }
  ...
  std::cout << "xc_mgga_r2scan_device: points=" << rho.size()
            << " stride=" << stride
  ...
  if (!all_wv_rho_match || !all_wv_grad_match || !all_wv_tau_match ||
    ...
    return Fail("device r2SCAN weighted outputs differ from the libxc oracle");
  }
  return 0;
}
