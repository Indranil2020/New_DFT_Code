...
int main() {
  ...
  for (std::size_t point = 0; point < rho.size(); ++point) {
    ...
    max_wv_rho_relative_error = std::max(
        ...
        RelativeError(observed_wv_rho[point], weights[point] * vrho));
    ...
    for (std::size_t component = 0; component < 3; ++component) {
      max_wv_grad_relative_error = std::max(
          ...
          RelativeError(observed_wv_grad[component * stride + point],
  ...
  if (max_wv_rho_relative_error > kRung0RelativeTolerance ||
      max_wv_grad_relative_error > kRung0RelativeTolerance ||
